...
class Project2:
    ...
    def __init__(self):
        ...
        model = self.train_NN(self.nOarr, self.nlarr,testin,testou)
        model.save('Test.h5')                               ### 파일 이름 지정
            
        model = load_model('Test.h5')                        ### 파일 이름 지정
    ...
    def train_NN(self, nOarr, nlarr,testin,testou):
        ...
        model.summary()
        early_stop = EarlyStopping(monitor='loss', patience=1000, verbose=1)
        hist = model.fit(nOarr, nlarr, epochs = 500,
                  batch_size=50, verbose=0, callbacks=[early_stop],
                         validation_data=(testin, testou))
    ...
    def graf(self, model, testin, testou):
        ...
        d.to_csv('C:/Users/keuntae/Desktop/res_min.csv')
